...
void Text_write(Text *text,const char *fmt,...){
	...
	vsnprintf(p+text->n,n+1,fmt,va);	// writes the new chars in the buffer
	text->buf=p;	// updates the buffer pointer
	text->n+=n;	// updates the number of chars in the buffer
	va_end(va);
	}
